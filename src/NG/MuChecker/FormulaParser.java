...
public class FormulaParser {
    ...
    public FormulaParser(File formulaFile) throws FileNotFoundException {
        ...
        ParseReturn result = parseRecursive(formula.toString(), 0, MAX_PRIORITY, null, new ArrayList<>());
    ...
    public FormulaParser(String formula) {
        ParseReturn result = parseRecursive(formula, 0, MAX_PRIORITY, null, new ArrayList<>());
        assert result.end == formula.length();
     ...
     * @param bindingFixPoint    the fixed point that surrounds this portion of the formula
     * @param allParentFixpoints
     * @return {@code [formula, end]} with formula the parsed formula, and end the character where the parsing stopped.
     */
    private ParseReturn parseRecursive(
            String formula, int start, int currentPriority, FixedPoint bindingFixPoint,
            Collection<FixedPoint> allParentFixpoints
    ) {
        ...
        while (i < formula.length()) {
            ...
            if (firstChar == ' ') { // skip spaces
                ...
                ParseReturn right = parseRecursive(formula, i + 2, AND_OR_PRIORITY, bindingFixPoint, new ArrayList<>());
                ...
                if (smallestFixedPoint) {
                    fp = new SmallestFixedPoint(fixVarName, variableNumber++, bindingFixPoint, false);
                } else {
                    fp = new LargestFixedPoint(fixVarName, variableNumber++, bindingFixPoint, false);
                }

                fixedPoints.add(fp);
                fixedPointDict.put(fixVarName, fp);

                for (FixedPoint parent : allParentFixpoints) {
                    parent.addDescendant(fp);
                }

                allParentFixpoints.add(fp);
                ParseReturn right = parseRecursive(formula, i + 5, MAX_PRIORITY, fp, new ArrayList<>());
                allParentFixpoints.remove(fp);

                fp.setRight(right.formula);
                current = fp;
                ...
                assert current == null : String.format("current == '%s'", current);
                ParseReturn inside = parseRecursive(formula, start + 1, MAX_PRIORITY, bindingFixPoint, new ArrayList<>());
                ...
                ParseReturn right = parseRecursive(formula, closing + 1, DIAMOND_BOX_PRIORITY, bindingFixPoint, new ArrayList<>());
                ...
                if (fixedPoint == null) {
                    throw new IllegalArgumentException(String.format(
                            "Unknown character starting at '%s' on line %d",
                            formula.substring(i, i + 10), i
                    ));
                }
                FixedPointVariable variable = fixedPoint.variable;

                for (FixedPoint fx : allParentFixpoints) {
                    fx.addVar(variable);
                }
